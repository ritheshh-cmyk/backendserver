...
function registerRoutes(app, io) {
        ...
        (async () => {
            try {
                ...
                if (reportType === "pl") {
                    ...
                    const expensesByCategory = expenditures.reduce((acc, exp) => {
                        if (!acc[exp.category])
                            acc[exp.category] = 0;
    ...
    app.post('/api/send-sms', async (req, res) => {
        ...
        try {
            ...
            if (!fast2smsApiKey) {
                return res.status(500).json({ success: false, message: 'SMS API key not configured' });
            }
            // fast2sms API call
            ...
            if (data.return) {
                ...
                res.status(500).json({ success: false, message: 'SMS failed', data });
            }
        }
        catch (err) {
            res.status(500).json({ success: false, message: 'SMS error', error: String(err) });
        }
    });
